...
class SshSessionHostEventBridgeTest {
    @Test
    fun `attach twice fails`() {
        val bridge = SshSessionHostEventBridge(RecordingListener())
    ...
    fun `events dispatch in order after attach`() {
        ...
        val bridge = SshSessionHostEventBridge(listener)
    ...
    fun `listener failures are reported and isolated`() {
        val listener =
            object : SshEventListener by SshEventListener.NONE {
    ...
    private class RecordingListener : SshEventListener {
