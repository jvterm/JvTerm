...
- `DONE(input/session)`: host-bound interactive reports and delimiters for
  ...
  `:jvterm-input` and serialized through `TerminalSession` with core/parser
  responses.
...
The `:jvterm-input` module owns host-bound keyboard, mouse, paste, and focus
encoding. It depends on `:jvterm-protocol` vocabulary and core input-readable
...
`jvterm-input/docs/jvterm-input-contract.md`.
...
- Ownership:
    - `jvterm-protocol`: primitive constants for progressive-enhancement flags,
      flag-application modes, event types, and functional-key numeric codes used
      by host-bound encoding.
    - `jvterm-parser`: recognize Kitty keyboard mode controls only. It emits
    ...
    - `jvterm-core`: store input-readable Kitty keyboard flags and, once
    ...
    - `jvterm-host`: maps active flag application and stack controls
      to core APIs, with explicit no-op/TODO behavior for unsupported query
      responses until policy exists.
    - `jvterm-input`: add a dedicated Kitty encoder branch selected from core
...
- `DONE(host)`: transport/session split:
    - `:jvterm-transport-api` defines `TerminalConnector` and
      `TerminalConnectorListener`
    - `:jvterm-session` owns parser/core synchronization, core response
      draining, input encoding, outbound write ordering, local/remote close state,
      and idempotent parser cleanup
    - `:jvterm-testkit` provides `MockConnector` for logic tests
- `DONE(host)`: local PTY process lifecycle:
    - `:jvterm-pty` starts PTY4J-backed processes behind `PtyConnector`
...
- `DONE(host/ui)`: reusable `:jvterm-ui-swing` module exists with a public
...
- `DONE(host/ui)`: standalone `:jvterm-ui-swing-demo` application wires the
