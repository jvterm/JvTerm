...
- `TODO(session/host/ui)`: shell integration persistent command history, multiline command text extraction, command navigation, prompt click actions, command output selection, current-directory metadata, rich command metadata, non-PowerShell shell bootstraps (`bash`, `zsh`, `fish`, and shell-specific WSL/Git Bash handling), on top of the existing OSC 133 marker event backbone, PowerShell bootstrap, bounded single-line command text metadata, command timeline, and basic Swing prompt/failure decorations.
