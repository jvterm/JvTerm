...
- `TODO(host/ui)`: IntelliJ completion-provider wiring through a plugin-owned adapter, richer production ranking signals beyond session MRU recency/use/profile/directory context, Fig-style spec corpus import, profile/directory history indexing, live triggers, path/current-directory providers, and IntelliJ-native popup styling remain host-owned follow-up work. The reusable Swing terminal popup surface, keyboard/mouse selection, acceptance callback, range-aware Swing acceptance, dependency-free layered completion foundation, deterministic source merge/ranking/deduplication, bounded in-memory session MRU source, standalone spec/MRU provider wiring with request context, and standalone/IntelliJ enablement settings exist. Shell-returned completions are intentionally deferred.
