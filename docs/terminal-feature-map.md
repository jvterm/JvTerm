...
- **Pixel-Aware Swing Viewport**: One Swing-owned visual geometry model maps terminal rows, fractional scroll offsets, prompt-divider bands, hit testing, repaint bounds, command-navigation anchors, and terminal-pixel mouse coordinates. Scrollbar adapters consume visual pixel ranges, so prompt dividers and fractional component heights remain scrollable even when core history has zero rows.
