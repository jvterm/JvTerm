...
- **OSC 133 Shell Integration Markers**: FinalTerm/iTerm-compatible shell lifecycle marker events (`OSC 133 ; A/B/C/D ST`) are parsed and forwarded through host, PTY, and workspace event boundaries. Session-owned shell integration state stores bounded prompt rows and failed command ranges by absolute render row, and Swing can render configurable prompt dividers plus failed-command left gutter rails. Command history storage, command text extraction, navigation, prompt actions, current-directory metadata, and durable re-anchoring across resize/reflow remain separate follow-up features.
