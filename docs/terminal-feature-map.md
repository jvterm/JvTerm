...
- **Fixed-Grid Swing Viewport**: One Swing-owned geometry model maps terminal rows, fractional scroll offsets, hit testing, repaint bounds, command-navigation anchors, and terminal-pixel mouse coordinates while preserving exact fixed row pitch. Prompt dividers are zero-layout row-top decorations and never change visible row count, scroll range, mouse coordinates, or PTY dimensions.
