...
/**
 * Returns the first visible cell column covered by the cell at [column].
 *
 * @param flags public render cell flags for the cell at [column].
 * @param column zero-based visible cell column.
 * @return leading column for normal and wide-cell trailing positions.
 */
fun visualCellRangeStart(
    flags: Int,
    column: Int,
): Int = if (flags and TerminalRenderCellFlags.WIDE_TRAILING != 0) maxOf(0, column - 1) else column

/**
 * Returns the visible cell span covered by the cell at [column].
 *
 * @param flags public render cell flags for the cell at [column].
 * @param column zero-based visible cell column.
 * @param columns visible column count.
 * @return clipped visual span in cells.
 */
fun visualCellRangeSpan(
    flags: Int,
    column: Int,
    columns: Int,
): Int {
