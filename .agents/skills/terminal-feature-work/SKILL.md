...
1. Identify the owning layer:
   - `ketraterm-parser` recognizes byte/protocol syntax and emits semantic commands.
   - `ketraterm-core` owns grid physics, modes, width, attributes, storage, and state.
   - `ketraterm-host` maps existing parser semantics to existing core APIs.
